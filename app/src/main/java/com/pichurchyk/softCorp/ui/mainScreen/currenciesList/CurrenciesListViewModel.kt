...
@HiltViewModel
class CurrenciesListViewModel @Inject constructor(private val useCase: CurrenciesUseCase) :
    ViewModel() {
            ...
            when (val apiCall = latestCurrencies.await()) {
                is TaskResult.Success -> {
                    useCase.getAllLikedCurrencies().collectLatest { resultFromDb ->
                        ...
                        val resultList = mutableListOf<CurrencyRateItemDto>()
                        apiCall.value.getRates().forEach {
                            if (resultFromDbNames.contains(it.name)) {
                                val itemToEmit =
                                    it.mapToCurrencyRateItemDto().apply { isLiked = true }
                                resultList.add(itemToEmit)
                            } else {
                                resultList.add(it.mapToCurrencyRateItemDto())
                            }
                        }
                        _currencies.emit(resultList)
                    }
                }
    ...
    fun toggleFavoriteCurrency(currencyRateItem: CurrencyRateItemDto) {
        viewModelScope.launch(Dispatchers.IO) {
            if (currencyRateItem.isLiked) {
                ...
                useCase.addCurrencyToFavorite(currencyRateItem.mapToCurrencyRateItem())
            }
        }
    }
