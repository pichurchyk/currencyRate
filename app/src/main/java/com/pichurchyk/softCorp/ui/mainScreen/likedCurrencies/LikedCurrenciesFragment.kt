...
import com.pichurchyk.softCorp.databinding.FragmentLikedCurrenciesBinding
...
@AndroidEntryPoint
class LikedCurrenciesFragment : BaseFragment(R.layout.fragment_liked_currencies),
    CurrenciesListAdapter.ItemClick, BaseCodesAdapter.ItemClick, SortAdapter.ItemClick {

    private val binding by viewBinding<FragmentLikedCurrenciesBinding>()
    private val viewModel by viewModels<LikedCurrenciesViewModel>()
    ...
    private fun setupSortBySpinner() {
        val sortConstantsArray = mutableListOf(
            SortConstants.AZ.name, SortConstants.ZA.name, SortConstants.LOWEST.name,
    ...
    override fun toggleFavoriteCurrency(currencyRateItem: CurrencyRateItemDto) {
        viewModel.removeCurrencyFromFavorite(currencyRateItem)
    }
